...
export const getUserPayouts = async (req, res) => {
  try {
    ...
    if (!payouts.length) {
      return res
        .status(404)
        .json({ message: "No payout records found for this user" });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch payout details" });
  }
};
...
export const updatePayoutStatus = async (req, res) => {
  try {
    ...
    // }
    // ✅ Only check KYC when marking completed
    if (status.toLowerCase() === "completed") {
      ...
      if (!kycStatus || kycStatus.status !== "verified") {
        return res.status(400).json({
          success: false,
          message: "User KYC is not verified. Cannot complete payout.",
        });
      }
    }
    ...
    if (!payoutRecord) {
      return res
        ...
        .json({
          success: false,
          message: "No payout record found for this user",
        });
    }
    ...
    if (!payout) {
      return res
        .status(404)
        .json({ success: false, message: "Payout entry not found" });
    }
